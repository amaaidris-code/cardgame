-- Make absorb_atk/absorb_hp retroactive: they block & absorb the LAST hit
-- (undo it and convert its damage) just like reflect and block, instead of
-- arming a forward shield that absorbs the NEXT incoming hit.
...
BEGIN
    ...
    ELSIF p_skill_effect = 'absorb_atk' THEN
        -- retroactive: absorb the LAST hit and convert it to ATK (like reflect/block)
        v_self.reflect_mult := 0;
        v_self.absorb_mode := null;
        v_self.absorb_hits := 0;
        v_was_pending := (NOT v_self.last_consumed) AND COALESCE(v_self.last_damage, 0) > 0;
        ...
        IF v_was_pending THEN
            v_self.hp := LEAST(v_self.max_hp, COALESCE(v_self.last_hp_before, v_self.hp));
            v_absorbed := v_self.last_damage;
            v_self.temp_atk := v_self.temp_atk + v_absorbed;
        END IF;

    ELSIF p_skill_effect = 'absorb_hp' THEN
        -- retroactive: absorb the LAST hit and convert it to HP
        v_self.reflect_mult := 0;
        v_self.absorb_mode := null;
        v_self.absorb_hits := 0;
        v_was_pending := (NOT v_self.last_consumed) AND COALESCE(v_self.last_damage, 0) > 0;
        ...
        IF v_was_pending THEN
            v_self.hp := LEAST(v_self.max_hp, COALESCE(v_self.last_hp_before, v_self.hp));
            v_absorbed := v_self.last_damage;
            v_self.hp := v_self.hp + v_absorbed;
