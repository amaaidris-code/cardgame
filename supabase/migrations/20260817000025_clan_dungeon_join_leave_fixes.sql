-- ============================================================
-- Two clan-dungeon 400s on enter/leave.
-- 1) join: its "close stale solo lobby run" cleanup deleted the
--    very run being re-joined (player alone in a lobby run), so the
--    following select raised 'الزنزانة غير موجودة'. Exclude the
--    target run (r.id <> p_run_id) from that cleanup.
-- 2) leave: leaving a finished run used to raise 'الزنزانة انتهت'
--    creating a spurious 400. Finished is now a silent no-op.
-- ============================================================

-- join: don't let the cleanup delete the run being joined
...
begin
    ...
    -- أغلق أي غرفة لوبي مهجورة يشاركها اللاعب وحيدًا (عدا الغرفة المطلوبة الآن)
...
-- leave: a finished run is a silent no-op (no spurious 400)
...
begin
    ...
    if v_run.status = 'lobby' then
        ...
        update public.clan_dungeon_players set alive = false
        where run_id = p_run_id and player_id = v_me;
    end if;
    -- finished: لا شيء يُفعل (لا خطأ)
end; $function$;